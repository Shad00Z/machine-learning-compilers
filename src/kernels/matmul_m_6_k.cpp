...
void mini_jit::kernels::matmul_m_6_k(mini_jit::Kernel &kernel,
                                     int m,
                                     int k)
{
    // Prepare the kernel
    ...
    kernel.add_instr(base::mov(gpr_t::x19, k)); // K loop counter
    ...
    if (mLoopRemainder > 0)
    {
        ...
        kernel.add_instr(base::mov(gpr_t::x14, gpr_t::x19));
        // save base matrix pointers
