                                     ...
                                     int k)
{
    ...
    kernel.add_instr(base::mul(gpr_t::x22, gpr_t::x4, gpr_t::x6)); // ldb * 4
    kernel.add_instr(base::mul(gpr_t::x23, gpr_t::x5, gpr_t::x6)); // ldc * 4

    kernel.add_instr(base::mov(gpr_t::x20, gpr_t::x1)); // B
    kernel.add_instr(base::mov(gpr_t::x21, gpr_t::x2)); // C

    mini_jit::kernels::internal::generateNLoop(kernel, m, n, k);
...
void mini_jit::kernels::internal::generateNLoop(mini_jit::Kernel &kernel,
                                                int m,
                                                int n,
                                                int k)
{
    ...
    if (nLoopIterations > 0)
    {
        ...
        {
            mini_jit::kernels::internal::generateMnLoop(kernel, mLoopIterations, k);
        }
        ...
        if (mLoopRemainder > 0)
        {
            ...
            if (mLoopRemainder == 1)
            {
                mini_jit::kernels::internal::generateMn1Loop(kernel);
            }
        ...
        kernel.add_instr(base::add(gpr_t::x20, gpr_t::x20, gpr_t::x22, 0));
        kernel.add_instr(base::add(gpr_t::x21, gpr_t::x21, gpr_t::x23, 0));
...
void mini_jit::kernels::internal::generateMnLoop(mini_jit::Kernel &kernel,
                                                int mLoopIterations,
                                                int k)
{
    // prepare the kernel
